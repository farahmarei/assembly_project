//
// Created by Farah Marei on 27/03/2025.
//
/*
 ...
 */
     ...
     for (int i = 0; i < lineCount; i++) {
         ...
         if(line->mnemonic) {
             ...
             if(inst->type == INST_R) {
                 ...
                 // System instructions (e.g., ECALL)

                 int service_num = 0;
                 if (line->operands != NULL) {
                     // Convert the operand string to an integer (e.g., "ecall 1" -> 1)
                     service_num = atoi(line->operands);

                     // Log the parsed service number for debugging
                     printf("Parsed ECALL service number: %d\n", service_num);

                     // Warn if it's outside expected range
                     if (service_num < 0 || service_num > 15) {
                         fprintf(stderr, "Warning on line %d: Unknown ECALL service: %d\n", line->lineNo, service_num);
                     }
                 }

                 // Encode the service number in bits [15:3] and opcode in [2:0]
                 machineWord = ((service_num & 0x1FF) << 3) | (inst->opcode & 0x7);
             }
 ...
 int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-v") == 0)
            ...
            if (i + 1 < argc) {
                ...
                i++;  // Skip next argument (binary filename)
            } else {
                fprintf(stderr, "Error: -o switch requires a binary file name\n");
                exit(1);
            }
        }
        // Ensure filename is assigned only once
        else if (filename == NULL) {
            filename = argv[i];  // Take the first non-option argument
        }
    }
    ...
    // If no binary file name provided, derive it from the source file name
    if (binFilename == NULL) {
        char temp[256];
        strcpy(temp, filename);
        char *dot = strrchr(temp, '.');  // Find last dot in filename
        if (dot)
            strcpy(dot, ".bin");  // Replace extension
        else
            strcat(temp, ".bin");  // Append .bin if no extension found
        binFilename = strdup(temp);
    }

    printf("Current Working Directory: %s\n", getenv("PWD"));  // Debugging
